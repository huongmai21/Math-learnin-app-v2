...
export const getPosts = async (params = {}) => {
  try {
    ...
    throw new Error(error.response?.data?.message || "Không thể lấy danh sách bài đăng");
  }
};
...
export const toggleBookmarkPost = async (postId) => {
  try {
    ...
    const isBookmarked = bookmarks.data?.data?.some((bookmark) => bookmark.referenceId === postId) || false;
